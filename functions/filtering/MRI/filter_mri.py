...
def dates_filter(client,index, idx_list, threshold):
    ...
    mri_dic = {key['_id']:[] for key in dates} # dvoice_id : 
    mri_freq = {key['_id']:0 for key in mri} # mri_id : frequency of occurence
    dic = {key['_source']['dummy_id']:[] for key in dates} # dvoice_id trimmed : [all of the dates associated with the id]
    for key in dates:
...
def gen_combined(threshold):
    ...
    client = helper.get_client()  # acquire the client
...
#test_result = gen_combined([180])
#print(len(test_result['final']))
